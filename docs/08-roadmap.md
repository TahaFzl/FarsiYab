...
✅ **فاز ۳ تمام شد (۲۰۲۶-۰۹-۲۵):** همه‌ی شهرها بین ۳ تا ۹ سورس مستقل دارند ([گزارش](reports/phase3-coverage.md)).

## فاز ۴: جامعه و کیفیت
- [x] فرم ثبت کسب‌وکار (`/fa/submit`) و درخواست حذف یا اصلاح (حذف دائمی با `do_not_index`)
- [x] پنل ادمین (`/fa/admin`): صف بررسی، گزارش‌ها، وضعیت سورس‌ها، ایندکس دستی
- [x] برچسب‌گذاری ۳۰۰ نمونه و اصلاح Detector ([گزارش](reports/phase4-precision.md)); ⚠️ برچسب‌های این دور را Claude زده و باید یک نفر بازبینی کند
- [x] نمایش روی نقشه (Leaflet + OSM)
- [x] صفحه‌های SEO شهر و دسته، `sitemap.xml` و `robots.txt`

**معیار اتمام:** دقت (precision) Detector روی نمونه‌های برچسب‌خورده حداقل ۹۰٪ در برچسب‌های «بالا» و «متوسط».

✅ **فاز ۴ تمام شد (۲۰۲۶-۰۹-۲۶):** دقت «بالا + متوسط» قبل از اصلاح ۹۱٫۶٪ و بعد از اصلاح ۹۹٫۴٪ ([گزارش](reports/phase4-precision.md)).

## فاز ۵: گسترش
- [x] **هر شهری در دنیا** (پیشنهاد با Photon، ساخت شهر در اولین استفاده با Nominatim)، به‌علاوه‌ی ۱۰ شهر منتخب جدید در بریتانیا، سوئد، هلند، فرانسه، استرالیا، ترکیه و امارات، با آستانه‌ی جدا برای ترکیه و امارات ([گزارش](reports/phase5-coverage.md))
- [x] دایرکتوری‌های کشورهای جدید بررسی شدند؛ هیچ‌کدام اجازه‌ی خواندن نمی‌دهند (فقط لینک). به‌جای آن‌ها ثبت خیریه‌های بریتانیا و استرالیا و ثبت شرکت‌های فرانسه (SIRENE) اضافه شدند
- [x] بررسی ماهانه‌ی کانال‌های تلگرامی که سورس‌ها پیدا کرده‌اند (فقط عنوان و توضیحات)
- [x] ادعای مالکیت کسب‌وکار بدون حساب (کد در وب‌سایت یا کانال تلگرام، یا بررسی دستی ادمین)
