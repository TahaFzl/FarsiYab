...
### `GET /places?q=gothen&lang=fa` ✅ فاز ۵
پیشنهاد شهر در حین تایپ (Photon). نتیجه: `[{ osm_id: "N25930131", name, state, country, country_code }]`. چیزی ذخیره نمی‌شود.

### `POST /cities` ✅ فاز ۵
`{ "osm_id": "R2775550" }` → `{ slug, country, name }`. شهر را در اولین استفاده از Nominatim می‌سازد (یا شهر موجودی را که آن نقطه را پوشش می‌دهد برمی‌گرداند). `422` برای مکانی که شهر نیست یا در کشورهای فارسی‌زبان است؛ `429` بعد از سقف روزانه.
