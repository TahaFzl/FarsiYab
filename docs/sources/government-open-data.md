...
### کشورهای فاز ۵
| شناسه | دیتاست | لایسنس | نکات |
|---|---|---|---|
| `gov:uk_charities` | Charity Commission for England and Wales، فایل کامل ثبت خیریه‌ها (`publicextract.charity.zip`، حدود ۵۷ مگابایت فشرده و ۵۰۰ مگابایت JSON) | Open Government Licence v3.0 | فقط خیریه‌های `Registered` اصلی (نه شاخه‌های `linked`). فایل خط‌به‌خط خوانده می‌شود و کل آن در حافظه نمی‌ماند. وب‌سایت خود خیریه به‌عنوان لینک ذخیره می‌شود تا بررسی وب‌سایت آن را بخواند. لینک مدرک، صفحه‌ی همان خیریه در ثبت رسمی است. آزمایش ۲۰۲۶-۰۹-۲۶: ۴۳ نام ایرانی در کل کشور |
| `gov:acnc_charities` | ACNC Register (CSV روی data.gov.au) | CC BY 3.0 AU | «نام‌های دیگر» سازمان هم خوانده می‌شود (مثلاً «Persian Language School WA» زیر نام حقوقی «Iranian Association Inc»). آزمایش: ۱۳ نام ایرانی در کل کشور |
| `gov:fr_sirene` | API دولتی جست‌وجوی شرکت‌ها روی ثبت SIRENE (`recherche-entreprises.api.gouv.fr`) | Licence Ouverte 2.0 | بدون کلید، حداکثر ۷ درخواست در ثانیه. جست‌وجوی نام با کلمات کلیدی (persan، iranien، شهرهای ایران و ...) در هر département (`regions` در `cities.yaml`). **شرکت‌های شخصی (`nature_juridique` 1xxx) کنار گذاشته می‌شوند**، چون نامشان نام خود شخص است. رکوردهایی که `statut_diffusion` آن‌ها غیر از `O` است هم کنار گذاشته می‌شوند (عمومی نیستند). ⚠️ شبکه‌ی محیط توسعه به این API دسترسی ندارد، پس فقط شکل پاسخ از بیرون بررسی شد و تست با داده‌ی نمونه انجام شد |

هلند (KvK) و سوئد (Bolagsverket) دانلود رایگان بدون حساب ندارند. ترکیه و امارات هم داده‌ی باز مشابهی ندارند.
