...
> وضعیت فعلی: **فاز ۱ (بک‌اند) و فاز ۲ (سایت) انجام شدند.** اجرا: [`services/api`](services/api/README.md) و [`apps/web`](apps/web/README.md). راه‌اندازی سرور: [`deploy/`](deploy/README.md). فاز بعدی: سورس‌های بیشتر ([نقشه‌ی راه](docs/08-roadmap.md)).
...
- **صف کارها:** جدول `job` در خود PostgreSQL (بدون Redis و بدون Docker)
